...
From source (dev + web console deps):

```bash
git clone https://github.com/findthehead/EEG.git
cd EEG
pip install -e ".[dev]"
pip install -r requirements.txt   # needed for eeg --serve
```
...
# Full OSS web UI (requires requirements.txt)
...
Runs the Django OSS application (`manage.py runserver` under the hood). Install `requirements.txt` first.
...
Web UI and org features live alongside under `apps/`, `templates/`, `manage.py` (OSS console via `--serve`).
...
**`eeg --serve` import/Django errors** — install `requirements.txt` in addition to the PyPI package.
